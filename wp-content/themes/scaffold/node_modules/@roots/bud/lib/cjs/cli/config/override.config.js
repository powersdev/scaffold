...
const config = async (command) => {
    ...
    if (!isUndefined(command.src)) {
        ...
        command.app.children?.every((_name, child) => child.setPath('@src', command.src));
    }
    if (!isUndefined(command.dist)) {
        command.app.setPath('@dist', command.dist);
        command.app.children?.every((_name, child) => child.setPath('@dist', command.dist));
    }
    if (!isUndefined(command.publicPath)) {
        command.app.setPublicPath(command.publicPath);
        command.app.children?.every((_name, child) => child.setPublicPath(command.publicPath));
    }
    if (!isUndefined(command.manifest)) {
        command.app.store.set('features.manifest', command.manifest);
        command.app.children?.every((_name, child) => child.store.set('features.manifest', command.manifest));
    }
    if (!isUndefined(command.cache)) {
        command.app.api.call(`persist`, command.cache);
        await Promise.all(command.app.children.getEntries().map(async ([_name, child]) => {
